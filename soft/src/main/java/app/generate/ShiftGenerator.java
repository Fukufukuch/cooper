...
public class ShiftGenerator {
    private final int generateDays;
    ...
    public ShiftGenerator(List<TimeSlot> timeSlots, List<Position> positions, List<Worker> workers, Option option) {
        ...
        this.option = option;
        this.generateDays = option.getGenerateDays();
        this.firstDate = option.getFirstDate();
    }

    public Map<LocalDate, Map<TimeSlot, Map<Position, List<String>>>> generate() {
        Map<LocalDate, Map<TimeSlot, Map<Position, List<String>>>> shift = new LinkedHashMap<>();

        for (int d = 0; d < generateDays; d++) {
            LocalDate currentDate = firstDate.plusDays(d);
            Map<TimeSlot, Map<Position, List<String>>> dayMap = new LinkedHashMap<>();
            shift.put(currentDate, dayMap);

            for (TimeSlot currentSlot : timeSlots) {
                Map<Position, List<String>> slotMap = new LinkedHashMap<>();
    ...
    // phase1:ポジション別責任者割当
    private void phase1_AuthorityForPos(TimeSlot currentSlot, LocalDate currentDate, Map<TimeSlot, Map<Position, List<String>>> dayMap, Map<Position, List<String>> slotMap) {
        List<Position> minSortedPositions = sortListByMinWorkers(positions);

        for (Position currentPosition : minSortedPositions) {
            List<String> shiftWorkerList = slotMap.computeIfAbsent(currentPosition, k -> new ArrayList<>());
    ...
    // phase2:時間帯別責任者割当
    private void phase2_AuthorityForSlot(TimeSlot currentSlot, LocalDate currentDate, Map<TimeSlot, Map<Position, List<String>>> dayMap, Map<Position, List<String>> slotMap) {
        List<Position> minSortedPositions = sortListByMinWorkers(positions);
    ...
    // phase3:最少人数割当メソッド
    private void phase3_PosMin(TimeSlot currentSlot, LocalDate currentDate, Map<TimeSlot, Map<Position, List<String>>> dayMap, Map<Position, List<String>> slotMap) {
    ...
    // phase4:新人制約割当メソッド
    private void phase4_Newcomer(TimeSlot currentSlot, LocalDate currentDate, Map<TimeSlot, Map<Position, List<String>>> dayMap, Map<Position, List<String>> slotMap) {
        ...
        for (Position currentPosition : maxSortedPositions) { // ポジション最大必要人数が多いものから順に入れる
            ...
            List<String> shiftWorkerList = slotMap.get(currentPosition);
    ...
    // phase5:通常割当メソッド
    private void phase5_Normal(TimeSlot currentSlot, LocalDate currentDate, Map<TimeSlot, Map<Position, List<String>>> dayMap, Map<Position, List<String>> slotMap) {
    ...
    private void vailDate(TimeSlot currentSlot, LocalDate currentDate, Map<TimeSlot, Map<Position, List<String>>> dayMap, Map<Position, List<String>> slotMap) {
        ...
        for (Position currentPosition : positions) {
            List<String> shiftWorkerList = slotMap.getOrDefault(currentPosition, List.of());
    ...
    // 労働者追加処理メソッド
    private void assign(Worker worker, TimeSlot slot, List<String> shift) {
    ...
    // 警告枠追加メソッド
    private void addWarningSlot(LocalDate date, TimeSlot slot, Position position, int nonconformTag, List<String> warningWorkers, WarningType warningType) {
    ...
    // 割当可能従業員リスト取得メソッド
    public static List<Worker> assignWorkers(LocalDate date, List<Worker> workers, Position position, TimeSlot slot, Map<TimeSlot, Map<Position, List<String>>> dayMap, List<String> shiftWorkerList, Option option) {
    ...
    // 割当可能判定メソッド
    public static boolean canAssign(LocalDate date, Worker w, Position position, TimeSlot slot, Map<TimeSlot, Map<Position, List<String>>> dayMap, List<String> shiftWorkerList, Option option) {
    ...
    // 二重割当確認メソッド
    public static boolean isCovered(Map<TimeSlot, Map<Position, List<String>>> dayMap, TimeSlot timeSlot, Worker worker) {
        Map<Position, List<String>> slotMap = dayMap.get(timeSlot);
        for (List<String> ids : slotMap.values()) {
    ...
    // 先輩必要判定メソッド
    private boolean hasSeniorRequired(Map<Position, List<String>> slotMap, Option option) {
        return countSeniorAssigned(slotMap) < option.getRequiredSeniorWorkers();
    }

    // 先輩割当数カウントメソッド
    private int countSeniorAssigned(Map<Position, List<String>> slotMap) {
        int count = 0;
        for (List<String> ids : slotMap.values()) {
            for (String id : ids) {
    ...
    // 時間帯責任者割当数カウントメソッド
    private int countAuthorityAssignedBySlot(Map<Position, List<String>> slotMap) {
        int count = 0;
        for (List<String> ids: slotMap.values()) {
            for (String id : ids) {
                Worker w = workerMap.get(id);
    ...
    private int countAuthorityAssignedByPosition(Position currentPosition, Map<Position, List<String>> slotMap) {
        int count = 0;
        for (String id : slotMap.getOrDefault(currentPosition, List.of())) {
            Worker w = workerMap.get(id);
            if (w != null && w.isHasAuthority()) count++;
        }
        return count;
    ...
    // 不適合タグ割当確認メソッド
    public static boolean isNonconformTagAssigned(List<String> shiftWorkerList, List<Worker> workers, int nonconformTag) {
        if (hasNonconformTagWorkersList(shiftWorkerList, workers, nonconformTag).size() > 1) return true;
        else return false;
    }

    // 不適合タグ該当従業員リスト取得メソッド
    public static List<String> hasNonconformTagWorkersList(List<String> shiftWorkerList, List<Worker> workers, int nonconformTag) {
        List<String> result = new ArrayList<>();

        for (String id : shiftWorkerList) {
            for (Worker w : workers) {
                if (w.getId().equals(id) && w.getNonconformTags().contains(nonconformTag)) {
                    result.add(id);
                    break;
                }
            }
